//This copy of the todoUtils contains profileBlocks, which are used by the 
//XPagesToolbox to monitor application performance
//To enable this feature, change the todotheme.theme to use the Profiled
//version of todoUtils: <script src="/todoUtilsProfiled.jss" clientSide="false"></script>
...
var getDBObj = function (doc) {
	if( null === requestScope.dataServiceDatabase) {
		if (bluemixContext.isRunningOnBluemix()) {
			if (bluemixContext.isHybridApplication()) {
				__profile("getDBObj", "hybrid use case - get database"){
					//Hybrid use case - replace "tododata.nsf" with the path of your data nsf
				...
				//Database service use case
				__profile("getDBObj", "normal bluemix - get database"){
...
var getIncompleteCount = function() {
	__profile("getIncompleteCount"){
		...
		if(null != dbObj) {
			...
			if(null != todos && "" !== todos) {
				__profile("getIncompleteCount", "todos.getEntryCount"){
					todoCount = todos.getEntryCount();
				}
			}
			if(null != complete && "" !== complete) {
				__profile("getIncompleteCount", "complete.getEntryCount"){
